...
data.frame(
  p_hat = c(
    rnorm(num_samples, mean = p, sd = sqrt(p * (1 - p) / 25)),
    rnorm(num_samples, mean = p, sd = sqrt(p * (1 - p) / 1000)),
    rnorm(num_samples, mean = p, sd = sqrt(p * (1 - p) / 100))
  ),
  scenario = c(
    rep(" ", num_samples),
    rep("  ", num_samples),
  ...
  theme_classic() +
  labs(
    x = "Proportion of candies that were orange",
    y = ""
  )
...
$$
SE_{\hat p} = \sqrt{\frac{p (1 - p)}{n}} = \sqrt{\frac{0.5(0.5)}{25}}=\blank
$$
...
The distribution of sample proportions $\hat{p}$ is called the _________________________. In real life it is not actually observed, because we typically only collect one sample, so we should be aware that there is variability in $\hat{p}$.

If we were to collect more samples (repeat the sampling process many times):

- The mean of the $\hat{p}$'s across many samples should be **closer** / **further** to $p$.
- The observed standard deviation of the $\hat{p}$'s should be **closer** / **further** to $\sqrt{p(1-p)/n}$.

If we collect more observations per sample (larger sample size $n$):

- The center (mean) of the sampling distribution of $\hat{p}$'s **stays the same** / **changes**.
- The standard error of $p$ should be **smaller** / **larger**, meaning **more** / **less** precise estimates of $p$.

\newpage

## When is $\hat p$ approximately normal?

The sampling distribution of $\hat{p}$ tends to be approximately normal when the following two conditions are met:
...
Is the success–failure condition satisfied? (Why?):
